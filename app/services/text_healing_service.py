...
SCAN_ARTIFACTS_MAP: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"(?i)\bc[ée\ufffd]dula\s+oe\b|\bc[\ufffd\?i]jla\s+oe\b|\bc[\ufffd\?i]dla\s+oe\b"), "CÉDULA DE"),
    (re.compile(r"(?i)\bc[íi]lidaoania\b|\bciudaania\b|\bciuoadania\b|\bctlidadanla\b|\bctljdadanla\b"), "CIUDADANÍA"),
    (re.compile(r"(?i)\bage3[íi]idos\b|\bage3idos\b|\bapefl!dos\b|\bapefl\?dos\b"), "Apellidos:"),
    (re.compile(r"(?i)\bnombrs\b|\bnom\?tes\b|\bnom[\ufffd\?]tes\b"), "Nombres:"),
    ...
    (re.compile(r"(?i)\bidentificaceen\.?\b"), "Identificación:"),
    (re.compile(r"(?i)\bacta\s+entrega\s+oe\b"), "ACTA DE ENTREGA DE"),
    (re.compile(r"(?i)\bmedic[\ufffd\?a-z]*meutos\b"), "MEDICAMENTOS"),
    (re.compile(r"(?i)\bdispositivos\s+[\ufffd\?a-z]*dicos\b"), "DISPOSITIVOS MÉDICOS"),
    (re.compile(r"(?i)\bpunto\s+sabana\s+2026\b"), "Punto Sabanalarga 2026"),
    ...
    (re.compile(r"(?i)\bbedlna\s*bercado\s*mry\b"), "MEDINA MERCADO MIRYAN"),
    ...
    (re.compile(r"(?i)\bsartalu\s*50\s*tab\b|\bsartan\s*50\s*tab\b"), "LOSARTAN 50 mg TABLETAS"),
    (re.compile(r"(?i)\bsartalu\b|\bsartan\b"), "LOSARTAN"),
    (re.compile(r"(?i)\borocloratiazida\s*tab\s*ca3a\b|\borocloratiazioa\s*tab\s*caja\b"), "HIDROCLOROTIAZIDA TAB CAJA"),
    (re.compile(r"(?i)\borocloratiazida\b|\borocloratiazioa\b"), "HIDROCLOROTIAZIDA"),
    (re.compile(r"(?i)'?oroxicio\s*6\s*frasco\s*360|\bdroxicio\s*6|\boxido\s+de\s+aluminio\s+6\s*gr"), "HIDROXIDO DE ALUMINIO 6% FRASCO"),
    ...
    (re.compile(r"(?i)\bcoosaluc\)\s*prohotora\s*oe\b|\bcoosalu[od0]\s+pro[am]otora\b"), "COOSALUD PROMOTORA DE"),
    (re.compile(r"(?i)\bcoosaluc\b|\bcocnlud\b|\bcoosaluo\b|\bcoosaluco\b"), "COOSALUD"),
    (re.compile(r"(?i)\bprohotora\s*oe\b"), "PROMOTORA DE"),
    (re.compile(r"(?i)\bce>?iins[\ufffd\?aá]?\b|\bcemitsa\b"), "CEMINSA"),
    (re.compile(r"(?i)\bedicina\s+genera[l]?\b"), "MEDICINA GENERAL"),
    (re.compile(r"(?i)\buna\s+margarita\b|\bedico\.\s*lina\s*gornaz\b|\blina\s*gornaz\b"), "Dra. Lina Margarita Gómez"),
    (re.compile(r"(?i)\bcantro\s*04\s*-\s*sede\s*praso\b|\baenci[\ufffd\?oó]m:\s*04\s*-\s*sede\s*paraso\b"), "Centro 04 - SEDE PRADO"),
